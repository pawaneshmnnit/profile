...
## 2b. Add the paper teaser / architecture images

Each Research entry shows an image on the left (like the reference site).
Create an **`images/`** folder in the repo root and add these files:

| File                      | Paper                                         | Needed?  |
| ------------------------- | --------------------------------------------- | -------- |
| `images/deft-dpt.png`     | DEFT-DPT (TCSVT)                              | yes      |
| `images/egohang.png`      | EgoHAnG (ICPR 2026)                           | yes      |
| `images/rgb-depth.png`    | Action Recognition RGB + Depth (RAIT)        | yes      |
| `images/poster.png`       | Sodh Sangam poster                           | optional |

- Recommended size: about **800×500 px** (a 16:10 crop of your architecture figure).
- PNG or JPG both work — just keep the filename exactly as above.
- Until a file exists, that card shows a neat labelled placeholder (no broken image),
  so you can upload the site first and add images later.

To use different filenames, edit the `src="images/..."` paths in `index.html`.
